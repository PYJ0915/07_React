// JSX 예제
import test1 from "../assets/hobee-logo2.png"
...
// JSX 예제 4 : 로그인 여부에 따라 화면 다르게 보이기 (삼항연산자)
const Exam5_4 = () => {
  // JS영역 -> if-else 사용 가능
  return <h1>{true ? '환영함다!' : '로그인해주세요.'}</h1>
}
...
const Exam5_5 = () => {
  ...
  // JS 내장 함수 중 배열의 요소에 하나씩 순차 접근에서 콜백함수 내에 있는 내용 수행 후 새로운 배열을 만들어 반환 -> map
  const listItem = members.map((member, index) => <li key={index}>{member}</li>);

  // Each child in a list should have a unique "Key" prop
  // -> React에서 동적으로 list를 렌더링할 때
