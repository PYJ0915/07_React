...
export default function Manager() {
  ...
  // 이메일, 닉네임, 전화번호 객체 하나로 상태 관리
  const [form, setForm] = useState({ email: "", nickname: "", tel: "" });
  const [accountList, setAccountList] = useState([]);
  const [isLoading, setIsLoading] = useState(true); // 서버에서 로딩 중이라고 초기값 세팅

  // 객체 형태인 상태 form 변경 함수
  ...
  const createAdminAccount = async () => {
    ...
    try {
      ...
      if (response.status == 201) {
        ...
        alert(`발급된 비밀번호는 ${result} 입니다. 다시 확인할 수 없으니 저장해두시길 바랍니다.`)
        console.log(result);
      }
  ...
  return (
    <>
      <div className="manager-div">
        ...
        {isLoading ? (<h1>Loading...</h1>) : (<section className="manager-section">
          <h2>관리자 계정 목록</h2>
